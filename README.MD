  ...
  alerts:
    ...
    - name: 'example'
      ...
      - alert: 'DrivePendingSectors'
        expr: 'sort_desc(netdata_smartd_log_reallocated_sectors_count_sectors_average{instance=~"(example[0-9]+)"}) > 1'
        for: '1h'
        labels:
          severnity: 'warning'
        annotations:
          summary: '{% raw %}{{ $labels.instance }} have drive with pending sectors{% endraw %}'
